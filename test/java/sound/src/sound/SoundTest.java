...
//  record();
  ...
  AudioInputStream ais = AudioSystem.getAudioInputStream(new File("C:\\Documents and Settings\\wml\\My Documents\\My Music\\À’ÿÌ -  ß√ﬂ÷¢.wav"));
  ...
  while((len=ais.read(buf))!=-1) {
   ...
   if(ais.getFormat().getChannels() == 2) {
    if(ais.getFormat().getSampleRate() == 16) {
     waveformGraph.put((short) ((buf[1] << 8) | buf[0]));//◊Û…˘µ¿
//     waveformGraph.put((short) ((buf[3] << 8) | buf[2]));//”“…˘µ¿
    } else {
     waveformGraph.put(buf[1]);//◊Û…˘µ¿
     waveformGraph.put(buf[3]);//◊Û…˘µ¿

//     waveformGraph.put(buf[2]);//”“…˘µ¿
//     waveformGraph.put(buf[4]);//”“…˘µ¿
    }
   } else {
    if(ais.getFormat().getSampleRate() == 16) {
